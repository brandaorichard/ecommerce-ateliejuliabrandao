...
export default function Category3Preview() {
  ...
  const handleMobileClick = () => {
    if (isMobile) {
      const el = document.getElementById("ver-mais-link-semelhanca");
      if (el) {
        el.animate(
          [{ transform: "scale(1)" }, { transform: "scale(0.95)" }, { transform: "scale(1)" }],
          { duration: 200 }
        );
        setTimeout(() => navigate("/categoria3"), 200);
      } else {
        navigate("/categoria3");
      }
    } else {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 mt-10 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-light text-gray-800">Por Semelhança</h2>
        <motion.button
      ...
      {!loading && !error && previewBabies.length === 0 && (
        <div className="py-6 text-sm text-gray-600">
          No momento, todos os bebês a semelhança estão esgotados.
        </div>
      )}
