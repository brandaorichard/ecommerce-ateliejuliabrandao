import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import RebornCard from "./RebornCard";
import { IoIosArrowForward, IoIosArrowBack } from "react-icons/io";
import { motion, AnimatePresence } from "framer-motion";
import { useSwipeable } from "react-swipeable";
import { useRelatedProducts } from '../hooks/useRelatedProducts';

function useWindowWidth() {
  const [width, setWidth] = useState(typeof window !== "undefined" ? window.innerWidth : 1024);
  useEffect(() => {
    function handleResize() { setWidth(window.innerWidth); }
    window.addEventListener("resize", handleResize);
    return () => window.removeEventListener("resize", handleResize);
  }, []);
  return width;
}

export default function RelatedProducts({ currentBabyId }) {
  const navigate = useNavigate();
  const { relatedProducts, loading, error } = useRelatedProducts(currentBabyId, 6);
  
  const [mobileIndex, setMobileIndex] = useState(0);

  const width = useWindowWidth();
  const isBetween768And1100 = width >= 768 && width <= 1100;
  const isMobile = width < 768;

  // Resetar índice ao mudar dados
  useEffect(() => {
    setMobileIndex(0);
  }, [relatedProducts.length]);

  // Auto-slide mobile
  useEffect(() => {
    if (!isMobile || relatedProducts.length <= 2) return;
    const interval = setInterval(() => {
      setMobileIndex(prev =>
        prev + 1 < Math.max(relatedProducts.length - 1, 1) ? prev + 1 : 0
      );
    }, 5000);
    return () => clearInterval(interval);
  }, [isMobile, relatedProducts.length]);

  const handlePrev = () => setMobileIndex(prev => Math.max(prev - 1, 0));
  const handleNext = () =>
    setMobileIndex(prev =>
      Math.min(prev + 1, Math.max(relatedProducts.length - 2, 0))
    );

  const swipeHandlers = useSwipeable({
    onSwipedLeft: () => handleNext(),
    onSwipedRight: () => handlePrev(),
    trackMouse: false,
  });

  return (
    <div className="max-w-6xl mx-auto px-4 mt-10 sm:px-6 lg:px-8">
      <div className="flex items-center justify-center mb-6">
        <h2 className="text-xl font-light text-black">
          Produtos Relacionados
        </h2>
      </div>

      {loading && (
        <div className="py-6 text-sm text-[#7a4fcf]">Carregando produtos relacionados...</div>
      )}
      {error && !loading && (
        <div className="py-6 text-sm text-red-600">Erro: {error}</div>
      )}
      {!loading && !error && relatedProducts.length === 0 && (
        <div className="py-6 text-sm text-[#616161]">
          Nenhum produto relacionado encontrado.
        </div>
      )}

      {/* MOBILE */}
      {!loading && !error && relatedProducts.length > 0 && (
        <div className="block md:hidden relative" {...swipeHandlers}>
          <div className="flex items-center justify-between">
            <button
              onClick={handlePrev}
              disabled={mobileIndex === 0}
              className="p-2 rounded-full text-[#7a4fcf] disabled:opacity-30"
            >
              <IoIosArrowBack size={26} />
            </button>

            <div
              className="flex-1 flex justify-center overflow-hidden gap-4"
              style={{ minHeight: 380, maxWidth: 380 }}
            >
              <AnimatePresence>
                {Array.from({ length: Math.min(2, relatedProducts.length) }).map((_, i) => {
                  const baby = relatedProducts[mobileIndex + i];
                  if (!baby) return null;
                  return (
                    <motion.div
                      key={`${baby.id}-${mobileIndex + i}`}
                      initial={{ x: 40, opacity: 0 }}
                      animate={{ x: 0, opacity: 1 }}
                      exit={{ x: -80, opacity: 0 }}
                      transition={{
                        type: "spring",
                        stiffness: 400,
                        damping: 40,
                        duration: 0.5,
                      }}
                      className="w-[160px]"
                    >
                      <RebornCard
                        baby={baby}
                        onClick={() => navigate(`/produto/${baby.slug}`)}
                        mini
                      />
                    </motion.div>
                  );
                })}
              </AnimatePresence>
            </div>

            <button
              onClick={handleNext}
              disabled={mobileIndex >= Math.max(relatedProducts.length - 2, 0)}
              className="p-2 rounded-full text-[#7a4fcf] disabled:opacity-30"
            >
              <IoIosArrowForward size={26} />
            </button>
          </div>
        </div>
      )}

      {/* DESKTOP */}
      {!loading && !error && relatedProducts.length > 0 && (
        <div
          className="hidden md:grid gap-6"
          style={{
            gridTemplateColumns: isBetween768And1100
              ? "repeat(auto-fit, minmax(220px, 1fr))"
              : "repeat(5, minmax(0, 1fr))",
          }}
        >
          {relatedProducts.slice(0, 5).map(baby => (
            <div key={baby.id} className="w-full">
              <RebornCard
                baby={baby}
                onClick={() => navigate(`/produto/${baby.slug}`)}
                mini
              />
            </div>
          ))}
        </div>
      )}
    </div>
  );
}
